...
function custom_cart_grouping_shortcode()
{
    ...
    ?>
        <style>
            #grouping-form { max-width: 600px; margin: auto; padding: 20px; background: #f9f9f9; border-radius: 8px; }
            .cart-item { display: flex; align-items: center; margin-bottom: 10px; }
            .cart-item img { width: 50px; height: 50px; margin-right: 10px; }
            .group-box { margin-top: 10px; padding: 10px; background: #e3e3e3; border-radius: 5px; }
            #total-price { font-size: 18px; font-weight: bold; margin-top: 10px; }
            .print-options { margin-top: 10px; }
            .print-options label { display: block; margin: 5px 0; }
            .print-options-display { font-size: 0.9em; color: #666; margin-top: 5px; }
            .empty-cart-message { text-align: center; padding: 20px; background: #f9f9f9; border-radius: 8px; margin-top: 20px; }
        </style>

        <?php if (WC()->cart->is_empty()): ?>
                <div class="empty-cart-message">
                    <p>You don't have any products in your cart. <a href="<?php echo wc_get_page_permalink('shop'); ?>">Continue shopping</a></p>
                </div>
        <?php else: ?>
                <h3></h3>
                            ...
                            <?php foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item):
                                ...
                                ?>
                                    <div class="cart-item" 
                                         ...
                                         data-front-print="<?= esc_attr($front_print); ?>"
                                         data-back-print="<?= esc_attr($back_print); ?>">
                                        ...
                                        <div class="product-info">
                                            ...
                                            <div class="quantity-controls" style="margin: 5px 0;">
                                                ...
                                                <button type="button" class="qty-decrease" data-key="<?= esc_attr($cart_item_key); ?>">-</button>
                                                <input type="number" class="qty-input" data-key="<?= esc_attr($cart_item_key); ?>" value="<?= $cart_item['quantity']; ?>" min="1" style="width: 50px; text-align: center; margin: 0 5px;">
                                                <button type="button" class="qty-increase" data-key="<?= esc_attr($cart_item_key); ?>">+</button>
                                                <button type="button" class="remove-item" data-key="<?= esc_attr($cart_item_key); ?>" style="margin-left: 10px; background: red; color: white; border: none; padding: 2px 8px; border-radius: 3px;">Remove</button>
                                            </div>
                                            <div class="price-display">
                                                <span class="item-price">€<?= number_format($cart_item['data']->get_price() * $cart_item['quantity'], 2); ?></span>
                                            </div>
        ...
        jQuery(document).ready(function($) {
            ...
            function displaySavedGroups() {
                groupedProducts.forEach(group => {
                    ...
                    <h4>${group.name} (€${group.price.toFixed(2)}) 
                        <button type="button" class="remove-group" data-group="${group.name}" style="margin-left: 10px; background: red; color: white; border: none; padding: 2px 8px; border-radius: 3px; cursor: pointer;">Remove Group</button>
                    </h4>
                    <div class="group-products">
                        ${group.products.map(product => `
                            <div class="group-product-item" style="margin: 5px 0; padding: 5px; background: #f0f0f0; border-radius: 3px;">
                                ...
                                <button type="button" class="remove-product-from-group" data-group="${group.name}" data-product-name="${product.name}" style="margin-left: 10px; background: #ff6b6b; color: white; border: none; padding: 1px 6px; border-radius: 2px; cursor: pointer; font-size: 12px;">Remove</button>
                            </div>
                        `).join('')}
                    </div>
            ...
            $(document).on('click', '.remove-group', function() {
                if (confirm('Are you sure you want to remove this group? Products will be returned to the cart.')) {
            ...
            $(document).on('click', '.remove-product-from-group', function() {
                if (confirm('Remove this product from the group? It will be returned to the cart.')) {
                    let groupName = $(this).data('group');
                    ...
                    group.products.forEach(product => {
                        ...
                        <div class="product-info">
                            ...
                            <div class="quantity-controls" style="margin: 5px 0;">
                                ...
                                <button type="button" class="remove-item" style="margin-left: 10px; background: red; color: white; border: none; padding: 2px 8px; border-radius: 3px;">Remove</button>
                            </div>
                            <div class="price-display">
            ...
            function removeProductFromGroup(groupName, productName) {
                ...
                if (group) {
                    ...
                    if (productIndex !== -1) {
                        ...
                        <button type="button" class="remove-group" data-group="${groupName}" style="margin-left: 10px; background: red; color: white; border: none; padding: 2px 8px; border-radius: 3px; cursor: pointer;">Remove Group</button>`);
                        ...
                        <div class="group-product-item" style="margin: 5px 0; padding: 5px; background: #f0f0f0; border-radius: 3px;">
                            ...
                            <button type="button" class="remove-product-from-group" data-group="${groupName}" data-product-name="${p.name}" style="margin-left: 10px; background: #ff6b6b; color: white; border: none; padding: 1px 6px; border-radius: 2px; cursor: pointer; font-size: 12px;">Remove</button>
                        </div>
                    `).join('');
            ...
            $("#create-group").on("click", function() {
                ...
                if (selected.length === 0) {
                    alert("Please select at least one product to group.");
                    return;
                }
                ...
                <div class="group-products">
                    ${group.products.map(product => `
                        <div class="group-product-item" style="margin: 5px 0; padding: 5px; background: #f0f0f0; border-radius: 3px;">
                            <span>${product.name} x ${product.quantity} - €${(product.price * product.quantity).toFixed(2)}</span>
                            <button type="button" class="remove-product-from-group" data-group="${groupName}" data-product-name="${product.name}" style="margin-left: 10px; background: #ff6b6b; color: white; border: none; padding: 1px 6px; border-radius: 2px; cursor: pointer; font-size: 12px;">Remove</button>
                        </div>
                    `).join('')}
                </div>
                <div class="upload-section">
            ...
            $("#send-inquiry").on("click", function() {
                ...
                if (groupedProducts.length === 0) {
                    alert("Please create at least one product group before sending inquiry.");
                    return;
                }
                ...
                $.post("<?= admin_url('admin-ajax.php'); ?>", {
                    ...
                    if (response.success) {
                        ...
                        alert("Error submitting inquiry: " + (response.data || 'Unknown error'));
                    }
